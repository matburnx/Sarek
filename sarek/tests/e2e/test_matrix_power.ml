...
(******************************************************************************
 * E2E test for Sarek PPX - Vector Power 
 *
...
let vector_power =
  [%kernel
    fun (a : float32 vector)
      ...
      let value = a.(tid) in
      if tid < n then c.(tid) <- value *. value ]

let compute_expected size =
  Array.init size (fun i -> (float_of_int i) *. (float_of_int i))
